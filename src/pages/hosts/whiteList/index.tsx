...
const WhiteList = () => {
  ...
  const [newData, setNewData] = useState<WhiteListData[] | []>([]);

  const { data, total, loading, handleApiRequest } =
    useApi<handleApiRequestResponse>('/filters/white_list/');
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      const res = await handleApiRequest(`/filters/white_list/create/`, 'post', formDataToSave);

      if (res) {
        await handleApiRequest('/filters/white_list/', 'get');
      }
  ...
  const handleEdit = async (formData: RowData, _: RowData) => {
    ...
    try {
      ...
      if (res) {
        handleApiRequest('/filters/white_list/', 'get');
      }
    } catch (error) {
      console.log(error);
    }
  ...
  const handleDelete = async (_: RowData, row: RowData) => {
    const baseUrl = `/filters/white_list/delete/${row.pid}/`;

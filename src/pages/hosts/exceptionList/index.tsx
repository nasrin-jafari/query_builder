...
const ExceptionList = () => {
  ...
  const [newData, setNewData] = useState<ExceptionListData[] | []>([]);

  const { data, total, loading, handleApiRequest } =
    useApi<handleApiRequestResponse>('/filters/exclusion/');
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      const res = await handleApiRequest(`/filters/exclusion/create/`, 'post', formDataToSave);

      if (res) {
        await handleApiRequest('/filters/exclusion/', 'get');
      }
  ...
  const handleDelete = async (_: RowData, row: RowData) => {
    const baseUrl = `/filters/exclusion/delete/${row.pid}/`;
